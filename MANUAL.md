...
## 6 棋谱与复盘
### 6.1 复盘
+ 打开棋谱（或自行走完一局棋）后，点击棋谱工具栏中的“复盘”按钮，也可以使用菜单栏“棋谱 -> 复盘”。
+ 复盘会使用当前引擎另开一个进程，逐个局面进行分析，因此不会影响主界面的对弈和分析。开始复盘时会自动取消“引擎执红/引擎执黑/引擎分析”状态并停止引擎，等待 5 秒后再开始分析。弹出框中设置每步分析的时间（毫秒，最低 500，低于 500 会提示并取消）。
+ 复盘过程中，整个页面会进入 loading 状态并锁定，避免误操作；界面上会显示当前进度。
+ 复盘结果分为三列展示：`分数`（该步走完后的局势分）、`评价`（最佳/优秀/良好/不精确/失误/大漏）和 `正着`（引擎推荐的最佳着法）。鼠标悬停在“评价”列可查看损失分数。
+ 棋谱最上面的“开始局面”也会一并分析，并在 `分数` 列显示开局局面分。
+ 评价等级参考 chess.com，分为：最佳、优秀、良好、不精确、失误、大漏。
+ 复盘结束后会弹出汇总窗口，显示红黑双方各等级着法的数量以及准确率。
